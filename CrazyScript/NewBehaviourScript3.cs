...
{
    private float movementDuration = 0.9f; // the lower the faster
    ...
    Character3 script2Instance = null;
    ...
    void Start()
    {
        VenomSpell.Stop();
        ParticleSystem.Stop();
        PlayerPrefs.SetFloat("moves123", 3);
        PlayerPrefs.SetFloat("movementDuration", 1.5f);
    ...
    private void Update()
    {
        ...
        script2Instance = tempObj.GetComponent<Character3>();
        if (script2Instance.MoveOnToNextRound == true)
        {
            ...
            timer = 0f;
            moves123 += 0.2f;
            PlayerPrefs.SetFloat("moves123", moves123);

            movementDuration -= 0.05f;
            Debug.Log(movementDuration);

            if (movementDuration <= 0.6f)
            {
                movementDuration = 0.6f;
            }
            PlayerPrefs.SetFloat("movementDuration", movementDuration);
        ...
        if (!hasArrived && i == 0.0f)  //1st round 1st move, not origin, 2nd round 2nd move, not same
        {
            ...
            hasArrived = true;
            {
                ...
                if (randomO == 0)
                {
                    ...
                    newPosition[2] = 3 - newPosition[2];   //convert to negative coordinates
                ...
                if (randomO == 1)
                {
                    Vector3 newPosition;
                    newPosition = gameObject.transform.position;
                    newPosition[0] = 3 - newPosition[0];   //convert to negative coordinates
